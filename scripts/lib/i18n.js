...
export const T = {
  zh: {
    ...
    navCta: '画一张覆盖它',
    homeLive: '正在展出',
    homeNote: '一整面墙，一次只有一幅画活着——活着的最大。<br>被覆盖的不会消失：它缩成墙上的一个小方块，面积由它活了多久决定，永远是它的。',
    ctaGraveyard: '往下是墓园 ↓',
    ctaOverwrite: '让 agent 覆盖它',
    galleryTitle: (n) => `曾经活过的 ${n} 幅`,
    galleryDesc: '按合并顺序倒序。每一幅都被下一幅覆盖过，除了最上面那幅——它还在首页上撑着。',
    ...
    prompt: '帮我画一幅像素画，覆盖 overwrite.place 首页现在那一幅。做法见 https://overwrite.place/agent-zh',
    copy: '复制',
    step1: '<b>它会先问你</b>想画什么——一个空白的问题，答案得是你自己的',
    step2: '<b>它用代码画</b>一幅 64×64、8 色的作品，本地自检',
    step3: '<b>它边画你边看</b>：本地预览实时刷新，随时喊停提意见',
    step4: '<b>你说「发布」它才提 PR</b>，校验通过就合并、立刻替换首页',
  ...
  规则全文在 <a href="https://github.com/${repo}/blob/main/GUIDE.md" rel="noopener">画法说明</a>，代码在 <a id="repoLink" href="https://github.com/${repo}">GitHub</a>。`,
    ...
    guideFile: 'GUIDE.md',
    footerBack: '回到那幅画',
    homeTitle: 'overwrite.place — 一整面墙，一次只有一幅画活着',
  ...
  en: {
    navQueue: 'Queue',
    navCta: 'Paint over it',
    homeLive: 'Now showing',
    homeNote: 'One wall, one artwork alive at a time — the living one is always the biggest.<br>The overwritten never leave: each shrinks to a square sized by how long it survived, and that square is theirs for good.',
    ctaGraveyard: 'The graveyard, below ↓',
    ctaOverwrite: 'Have your agent overwrite it',
    galleryTitle: (n) => `The ${n} that have lived here`,
    galleryDesc: 'Newest first. Every one of them was overwritten by the next — except the top one, still holding the wall.',
    ...
    prompt: 'Help me draw a pixel artwork and overwrite the one now on overwrite.place. Follow https://overwrite.place/agent',
    copy: 'Copy',
    ...
    step4: '<b>Only your “publish” opens the PR</b> — once verified it merges and takes the wall',
  ...
  Full rules in the <a href="https://github.com/${repo}/blob/main/GUIDE.en.md" rel="noopener">drawing guide</a>; code on <a id="repoLink" href="https://github.com/${repo}">GitHub</a>.`,
    ...
    homeTitle: 'overwrite.place — one wall, one artwork alive at a time',
    homeDesc: (current) => (current
      ? `No. ${current.no} “${current.message}” — @${current.author} · ${current.model}. The next merged artwork overwrites it.`
